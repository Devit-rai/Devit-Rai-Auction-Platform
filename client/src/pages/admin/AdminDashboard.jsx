...
const NewListingToast = ({ auction, onClose }) => (
  <div className="flex items-start gap-3 bg-white border border-indigo-200 rounded-xl shadow-xl px-4 py-3 w-[300px]"
    ...
    <div className="w-8 h-8 rounded-lg overflow-hidden bg-indigo-50 flex-shrink-0 flex items-center justify-center mt-0.5">
      {auction?.image?.url
        ? <img src={auction.image.url} alt="" className="w-full h-full object-cover" />
    ...
    <button onClick={onClose} className="text-slate-300 hover:text-slate-500 transition flex-shrink-0">
      <XCircle size={14} />
    </button>
  </div>
);
...
const AdminDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] font-sans flex">
      ...
      <div className="fixed bottom-5 left-5 z-[9999] flex flex-col gap-2.5 pointer-events-none">
        {newListingToasts.map((t) => (
          <div key={t.toastId} className="pointer-events-auto"
            style={{ animation: "toastIn .3s cubic-bezier(.22,1,.36,1)" }}>
            <NewListingToast
              auction={t}
              onClose={() => setNewListingToasts((p) => p.filter((x) => x.toastId !== t.toastId))}
            />
          </div>
        ))}
      </div>
