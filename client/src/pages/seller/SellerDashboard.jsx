...
import {
  ...
  LogOut, User, Shield, Clock, CheckCircle, XCircle,
} from "lucide-react";
...
const StatCard = ({ label, value, icon: Icon, color = "indigo", sub }) => {
  const colorMap = { indigo: "bg-indigo-50 text-indigo-600", emerald: "bg-emerald-50 text-emerald-600", amber: "bg-amber-50 text-amber-600", red: "bg-red-50 text-red-500" };
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
      {Icon && <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${colorMap[color]}`}><Icon size={18} /></div>}
      <div>
...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    const joinRoom = () => {
      if (user?._id) {
        ...
        console.log("[Seller] Joined notification room:", user._id);
      }
    };
    joinRoom();
    ...
    return () => {
      ...
      if (user?._id) socket.emit("leaveUserRoom", user._id);
    };
  }, [user?._id]);
  ...
  useEffect(() => {
    ...
    const onNewBid = ({ auctionId, currentBid, bid }) =>
      setAuctions((prev) => prev.map((a) => {
        if (a._id !== auctionId) return a;
        return { ...a, currentBid, bids: bid ? [...(a.bids || []), bid] : a.bids };
      }));
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pendingCount: auctions.filter((a) => (a.approvalStatus || "Pending") === "Pending").length,
      rejectedCount: auctions.filter((a) => a.approvalStatus === "Rejected").length,
