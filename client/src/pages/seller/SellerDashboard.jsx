...
import {
  LayoutDashboard, List, TrendingUp, Gavel, LogOut,
  User, Shield, Clock, CheckCircle, XCircle, AlertCircle,
} from "lucide-react";
...
const SellerDashboard = () => {
  ...
  const [listFilter, setListFilter] = useState("all"); // all | Live | Upcoming | Ended | Pending | Approved | Rejected
  ...
  const stats = useMemo(() => {
    const totalRevenue = auctions.reduce((s, a) => s + (Number(a.currentBid) || 0), 0);
    const live         = auctions.filter((a) => a.status === "Live");
    const pending      = auctions.filter((a) => (a.approvalStatus || "Pending") === "Pending");
    const rejected     = auctions.filter((a) => a.approvalStatus === "Rejected");
    ...
    return {
      ...
      recentBids:    [...allBids].sort((a, b) => new Date(b.bidTime) - new Date(a.bidTime)).slice(0, 5),
  ...
  const tableAuctions = auctions.filter((a) => {
    ...
    if (listFilter === "Pending")  return (a.approvalStatus || "Pending") === "Pending";
    ...
    <div className="min-h-screen bg-[#F7F8FA] flex font-sans text-slate-900">
      ...
      <aside className="w-56 bg-white border-r border-slate-100 hidden lg:flex flex-col fixed h-full z-50">
        ...
        <nav className="flex-1 px-3 pt-2 space-y-0.5">
          <NavItem icon={LayoutDashboard} label="Dashboard"    active={true}  onClick={() => navigate("/seller-dashboard")} />
      ...
      <main className="flex-1 lg:ml-56 p-7 min-h-screen">
        ...
        {stats.rejectedCount > 0 && (
          <div className="mb-5 flex items-start gap-3 bg-red-50 border border-red-200 rounded-2xl px-5 py-3.5">
            <AlertCircle size={15} className="text-red-500 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-xs font-bold text-red-700">
                {stats.rejectedCount} listing{stats.rejectedCount > 1 ? "s" : ""} rejected by admin
              </p>
              <p className="text-[11px] text-red-500 mt-0.5">
                Please review and re-submit or contact support for more details.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-7">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats.chartData}>
                  ...
                  <YAxis axisLine={false} tickLine={false}
                    tick={{ fontSize: 11, fill: "#94a3b8" }} allowDecimals={false} />
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col">
            ...
            <button onClick={() => navigate("/bid-history")}
              ...
              {[
                ...
                <button key={value} onClick={() => setListFilter(value)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-bold transition border ${
                    listFilter === value
                      ? highlight === "amber" ? "bg-amber-500 text-white border-amber-500"
                        : highlight === "red" ? "bg-red-500 text-white border-red-500"
                  ...
                  <span className={`text-[10px] font-black min-w-[16px] h-4 px-1 rounded-full flex items-center justify-center ${
                    listFilter === value ? "bg-white/20 text-white" : "bg-slate-100 text-slate-500"
                  }`}>{count}</span>
                </button>
              ))}
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 text-left">
                  <th className="px-5 py-3.5 text-[11px] font-black text-slate-400 uppercase tracking-wide">Item</th>
                  <th className="px-5 py-3.5 text-[11px] font-black text-slate-400 uppercase tracking-wide">Current Bid</th>
                  <th className="px-5 py-3.5 text-[11px] font-black text-slate-400 uppercase tracking-wide text-center">Bids</th>
                  <th className="px-5 py-3.5 text-[11px] font-black text-slate-400 uppercase tracking-wide">Time Left</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="text-center text-slate-400 py-12 text-xs">
                  ...
                  return (
                    <tr key={item._id}
                      ...
                      <td className="px-5 py-3.5 text-xs text-slate-500 whitespace-nowrap">
                        {item.status === "Ended" ? "—" : getTimeRemaining(item.endTime)}
                      </td>
