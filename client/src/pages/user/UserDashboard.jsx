...
const ProfileDropdown = ({ userName, userRole, onLogout, navigate }) => {
  ...
  const roleColor = roleColors[roleKey] || roleColors.bidder;
  const roleLabel = userRole ? userRole.charAt(0).toUpperCase() + userRole.slice(1) : "Bidder";

  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 px-2 py-1 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-200 transition cursor-pointer"
      >
          ...
          <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl border border-slate-200 shadow-xl z-[100] overflow-hidden">
            ...
            <div className="px-4 py-3.5 bg-gradient-to-br from-indigo-50 to-slate-50 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-black text-slate-800 leading-tight capitalize">{userName}</p>
                  <span className={`inline-flex items-center text-[10px] font-bold px-1.5 py-0.5 rounded-md ${roleColor}`}>
                    {roleLabel}
                  </span>
                </div>
              </div>
...
const Navbar = ({ userName, userRole, onLogout, navigate, search, setSearch }) => (
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          {t.urgent ? (
            ...
            <span className="bg-amber-500 text-white text-[10px] font-bold px-2.5 py-1 rounded-full">Upcoming</span>
          )}
        </div>
      </div>
...
const CategoryBox = ({ icon: Icon, label, color, count, navigate }) => {
  ...
  return (
    ...
    >
      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:text-white ${palette[color]}`}>
        <Icon size={20} />
      </div>
      <div className="text-center">
        <p className="text-xs font-bold text-slate-700 group-hover:text-indigo-700 transition-colors">{label}</p>
        {count > 0 && <p className="text-[10px] text-slate-400 mt-0.5">{count} listings</p>}
      </div>
    </button>
  );
};
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    setSearchResults(
      auctions.filter((a) => a.title?.toLowerCase().includes(q) || a.category?.toLowerCase().includes(q)).slice(0, 5)
    );
  }, [search, auctions]);
      ...
      {searchResults.length > 0 && (
        <div className="fixed top-14 left-1/2 -translate-x-1/2 w-[480px] z-[200] bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
          {searchResults.map((item) => (
            <div
              key={item._id}
              onClick={() => { navigate(`/auction/${item._id}`); setSearch(""); }}
            ...
            >
              <img src={item.image?.url} className="w-9 h-9 rounded-lg object-cover flex-shrink-0 bg-slate-100" alt={item.title} />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-slate-800 truncate">{item.title}</p>
                <p className="text-[10px] text-slate-400">{item.category} · {fmt(item.currentBid || item.startingBid || 0)}</p>
              </div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.status === "Live" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                {item.status}
              </span>
            </div>
          ))}
          <div
            onClick={() => { navigate("/auctions"); setSearch(""); }}
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-8 space-y-10">
        ...
        <section className="grid lg:grid-cols-2 gap-6 items-stretch">
          <div className="bg-white rounded-2xl border border-slate-100 p-8 flex flex-col justify-between gap-6 relative overflow-hidden">
            ...
            <div className="relative space-y-4">
              <div className="inline-flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> Market Active · {live.length} Live
              </div>
              <div>
              ...
              <p className="text-slate-500 text-sm leading-relaxed max-w-sm">
                Welcome back, <span className="text-slate-800 font-bold capitalize">{userName}</span>. Discover high-value lots and track your winning bids in real-time.
              </p>
              <div className="flex gap-3 pt-2">
                ...
                >
                  Enter Auction Hall <ChevronRight size={15} className="group-hover:translate-x-0.5 transition-transform" />
                </button>
                <button
                  onClick={() => navigate("/auctions")}
          ...
          {live[0] ? (
            ...
            >
              <img src={live[0].image?.url} alt={live[0].title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-5">
                    <div>
                      ...
                      <p className="text-white font-black text-base">{fmt(live[0].currentBid || live[0].startingBid || 0)}</p>
                    </div>
                    <div>
                      <p className="text-white/40 text-[10px] mb-0.5">Time Left</p>
                      <p className={`font-black text-base ${getTime(live[0].endTime).urgent ? "text-red-400" : "text-white"}`}>
            ...
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 flex flex-col items-center justify-center text-center p-8 min-h-[380px]">
              ...
              <p className="text-sm font-bold text-slate-600">No live auctions</p>
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">Live Auctions</h2>
              <p className="text-xs text-slate-400 mt-0.5">{live.length} active right now</p>
            </div>
            <button onClick={() => navigate("/auctions")} className="flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-700 transition">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-dashed border-slate-200 py-14 text-center">
              <p className="text-sm font-semibold text-slate-500">No live auctions at the moment.</p>
            </div>
          ) : (
        ...
        <section>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-4">
            {cats.map((cat) => <CategoryBox key={cat.label} {...cat} navigate={navigate} />)}
          </div>
        </section>
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
            <div>
              <div className="inline-flex items-center gap-1.5 bg-white/10 text-white text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full mb-3">
                <Zap size={10} className="text-yellow-300" /> {live.length} Auctions Live Right Now
              </div>
              <h2 className="text-2xl font-black text-white leading-tight">Don't miss the next big win.</h2>
              <p className="text-indigo-200 text-sm mt-1">New items added daily. Place your bid before the clock runs out.</p>
            </div>
            <button
              onClick={() => navigate("/auctions")}
            ...
            >
              Browse Auctions <ChevronRight size={14} className="group-hover:translate-x-0.5 transition-transform" />
            </button>
          </div>
        </section>
